...
const Header = () => {
  ...
  const isActive = (link: any) => {
    ...
    if (
      link.name === "About Us" &&
      ["/about", "/quality-assurance", "/infrastructure"].includes(location.pathname)
    )
      return true;
    return false;
  };
      ...
      >
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            >
              <div className="relative p-3 rounded-xl bg-gradient-to-br from-gray-800 to-gray-700 shadow-lg group-hover:shadow-xl border border-gray-600">
                <img
                  ...
                  alt="Logo"
                  className="h-10 w-auto object-contain brightness-110 transition-transform group-hover:scale-105"
                />
              </div>
            </button>
              ...
              {navLinks.map((link) => (
                <div key={link.name} className="relative group">
                  {link.hasDropdown ? (
                        ...
                        {link.name === "Our Products" ? (
                          <>
                            ...
                            {products.map((product) => (
                              <DropdownMenuItem key={product.slug} asChild>
                                <button
                                  onClick={() =>
                                    handleNavigation(`/products#${product.slug}`)
                                  }
                                  className="w-full px-4 py-2 text-sm text-left text-gray-300 hover:bg-gray-700 hover:text-blue-400 rounded-lg"
                                >
                                  {product.name}
                                </button>
                              </DropdownMenuItem>
                          ...
                          aboutSections.map((section) => (
                            <DropdownMenuItem key={section.anchor} asChild>
                              <button
                                onClick={() => handleNavigation(section.anchor)}
                                className="w-full px-4 py-2 text-sm text-left text-gray-300 hover:bg-gray-700 hover:text-blue-400 rounded-lg"
                              >
                                {section.name}
                              </button>
                    ...
                    >
                      {link.name === "Whatsapp" ? <WhatsAppButton /> : link.name}
                    </button>
                  )}
                </div>
              ))}
            ...
            >
              {isMobileMenuOpen ? <X className="w-5 h-5 rotate-90" /> : <Menu className="w-5 h-5" />}
            </button>
          </div>
          ...
          {isMobileMenuOpen && (
            <div className="lg:hidden bg-gray-800 border-t border-gray-600 py-4 mt-3 rounded-b-xl shadow-lg">
              {navLinks.map((link) => (
                <div key={link.name} className="mb-2 mx-2">
                  <div className="flex justify-between items-center">
                    <button
                      ...
                      className={`flex justify-between items-center w-full py-3 px-4 text-sm font-medium rounded-lg text-white transition duration-200 ${
                        isActive(link)
                          ? "bg-blue-600 text-white"
                          : "hover:bg-gray-700 hover:text-blue-400"
                      }`}
                    >
                      <span>{link.name === "Whatsapp" ? <WhatsAppButton /> : link.name}</span>
                      {link.hasDropdown && (
                        <ChevronDown
